...
function parseIRCBroadcasts()
{
        global $globalDb, $SETTINGS;
        $fp = fopen($SETTINGS['irc_broadcast_logfile'], "rb");


        $offset = $SETTINGS['last_irc_offset'];
        $contents = file_get_contents($SETTINGS['irc_broadcast_logfile'], false, null, $offset);
        ...
        {
				$msg = preg_replace('/[[:^print:]]/', '', $arr_contents[$i]);
                $pos = strpos($msg, ';');
                if ($pos > 5)
                {
                	// ping format: [BC/NORTHERN_COALITION] POISION_KEVIN: Fleet up on...	
                        $timestamp = substr($msg, 0, $pos-1);
                        $tsnew = substr($timestamp, 6);
                        $message = substr($msg, $pos+14);
                        $message = substr($message, 0, strlen($message)-1);
						$message = str_replace("  ", " ", $message); // remove double spaces
						// remove the word broadcast
						$message = str_replace("BROADCAST", "BC", $message);
						// find occurance of BC
						$pos4 = strpos($message, "BC/");
						// after pos4 there should be the group name
						$groupname = substr($message, $pos4);
						...
						$bc_username = substr($msg, $pos3);
						$pos3 = strpos($bc_username, ":");
						$bc_username = substr($bc_username, 0, $pos3);

						$message = $globalDb->real_escape_string($message);
						$bc_username = $globalDb->real_escape_string($bc_username);
						$groupname = $globalDb->real_escape_string($groupname);
						
						$sql = "INSERT INTO irc_ping_history (timestamp, message, from_character,groupname) VALUES ('$tsnew', '$message', '$bc_username','$groupname') ";
						$globalDb->query($sql);
						
						if ($message != $last_msg) // prevent spamming
						{
		...
		if (sizeof($collect_messages))
		{
			// send $collect_messages to user
			$message = stripslashes(implode($collect_messages, "\n"));
			echo "Sending (joined) message to telegram users: '$message'\n";
			try {
				send_fleetbot_to_telegram($message);
			} catch (Exception $ignore) {
				echo "Error sending to telegram... But whatever, we do not need that, right?";
			}
		}
		echo "\nDone!\n";
	...
	if ($length < 0)
        ...
        // store $length as the latest offset in database
        $sql = "UPDATE settings SET svalue='$length' WHERE name='last_irc_offset' ";
        $globalDb->query($sql);

        fclose($fp);


	// update vb4.notice (displayorder: increasing number, persistent: 1, active: 1, dismissible: 1)
	// update vb4.noticecriteria (noticeid, in_usergroup_x, 9) for burning napalm members only
	
	// notices with order 40,41,42,43,44 are for broadcast messages
	// get the last 5 pings
/*	$sql = "SELECT timestamp, message, groupname, id FROM irc_ping_history ORDER BY timestamp DESC LIMIT 0,3";
	$res = $globalDb->query($sql);
	...
	while ($row = $res->fetch_array())
	{
		...
		$sql = "UPDATE phrase SET text = '$msg' WHERE varname = 'notice_" . $cnt . "_html' ";
...
function check_corp_api_keys()
{
    ...
    while ($row = $res->fetch_array())
    {
        ...
        if ($access_mask != intval($SETTINGS['corp_api_key_accessmask']))
        {
            ...
            echo "ERROR: Corp API KeyID: $key_id; with wrong access_mask: " . $api_key_status['mask'] . "\n";
            do_log("ERROR: Corp API KeyID: $key_id; with wrong access_mask: " . $api_key_status['mask'], 1);
            continue;
        }
...
{
	global $globalDb, $SETTINGS, $supers_titans_id, $carriers_dreads_id, $force_aux_id;
	...
	if ($xml && $xml->result)
	{
		...
		{				
			...
			if (!in_array($typeID, $supers_titans_id) && 
				!in_array($typeID, $carriers_dreads_id) &&
				!in_array($typeID, $force_aux_id))
			{
				// not a super carrier
				continue;
			}
