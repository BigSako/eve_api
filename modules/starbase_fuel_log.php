<?php
	...
	// check if user is allowed to edit starbase details
	$sql = "SELECT page_name, m.group_id FROM api_page_access a, group_membership m
	WHERE a.page_name='starbase_save' AND (a.group_id = m.group_id OR m.group_id = 2) AND m.user_id = $userid  AND m.state=0 ";
	$res = $db->query($sql);
	...
	{
		$row = $res->fetch_array();
		if ($row['page_name'] == 'starbase_save')
		{
			$isPOSAdmin = true;
		}
	}
...
AND d.itemID = moonID AND s.state <= 10 ORDER BY r.regionName, m.solarSystemName, d.itemName");
	...
	base_page_header('',"POS Fuel Log $moonName","POS Fuel Log $moonName", $list_of_starbases);

	echo "Back to POS Details for <a href=\"api.php?action=starbase_detail&itemID=$posItemID&locationID=$locationID&moonID=$moonID&corp_id=$corp_id\">$moonName</a><br /><br />";
	...
	// get fuel block and stront log for the last 60 days
	$sql = "SELECT curTimeStamp, stateTimestamp, onlineTimestamp, state, fuel_status, stront_status FROM starbase_log 
	WHERE locationID = $locationID and moonID = $moonID and corp_id = $corp_id
	AND TIMESTAMPDIFF(DAY, curTimeStamp, now()) < 60
	ORDER BY curTimeStamp DESC ";

	echo "<div>";

	$res = $db->query($sql);

	echo "<table><tr><th>Time</th><th>State</th><th>Fuel Blocks</th><th>Stront</th></tr>";

	while ($row = $res->fetch_array())
	{
		echo "<tr><td>" . $row['curTimeStamp'] . "</td><td>";
		$stateTimestamp = $row['stateTimestamp'];
		$onlineTimestamp = $row['onlineTimestamp'];
		...
		switch ($row['state'])
		{
			case 0:
				...
				break;
			case 1:
				$pos_state_text = "Anchored/Offline since $stateTimestamp";
				break;
			case 2:
				$pos_state_text = "Onlining until $onlineTimestamp";
				break;
			case 3:
				$pos_state_text = "Reinforced until $stateTimestamp";
				break;
			case 4:
				$pos_state_text = "Online";
				break;
		}
		echo $pos_state_text . "</td><td>" . $row['fuel_status'] . "</td><td>" . $row['stront_status'] . "</td></tr>";
	}
